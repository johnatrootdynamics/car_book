...
type StaffHome = { events: TrackEvent[] };

export default function HomeScreen() {
  const { account, api } = useAuth(); const [data, setData] = useState<DriverHome | StaffHome | null>(null); const [error, setError] = useState('');
  useEffect(() => { if (!account) return; const path = account.type === 'user' ? '/driver/dashboard' : account.type === 'employee' ? '/staff/events' : '/me'; api<any>(path).then(setData).catch(e => setError(e.message)); }, [account, api]);
  if (!account || (!data && !error)) return <Loading />;
  if (account.type === 'user' && data && 'stats' in data) return <Screen>
    ...
    <View style={styles.stats}>{[['Days', data.stats.events_attended], ['Tracks', data.stats.tracks_visited], ['Cars', data.stats.vehicles]].map(([label, value]) => <Card key={String(label)} style={styles.stat}><Text style={styles.statValue}>{value}</Text><Text style={styles.statLabel}>{label}</Text></Card>)}</View>
    <SectionTitle title="Upcoming events" />
    {data.upcoming_events.length ? data.upcoming_events.slice(0, 4).map(event => <EventRow key={event.id} event={event} />) : <Empty title="No events booked" detail="Your upcoming driver events will appear here." />}
    <SectionTitle title="Your garage" />
    {data.garage.length ? data.garage.slice(0, 3).map(car => <Card key={car.id}><Text style={ui.title}>{car.label}</Text><Text style={ui.body}>{car.color || 'Color not listed'}</Text></Card>) : <Empty title="Your garage is empty" detail="Add a car on the Track Ops website to use it at checkout." />}
  </Screen>;
  if (account.type === 'employee' && data && 'events' in data) return <Screen>
    ...
    {data.events.length ? data.events.slice(0, 6).map(event => <EventRow key={event.id} event={event} />) : <Empty title="No upcoming events" detail="New events will appear here." />}
  </Screen>;
  return <Screen><Hero eyebrow={account.type} title={`Welcome, ${account.name}`} subtitle="The first mobile release focuses on drivers and track operations." /><Empty title="More mobile tools are coming" detail="Your account and password controls are available in Profile." /></Screen>;
}
